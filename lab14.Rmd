...
bird_line <- group_summarize(birds, type)
...
ggplot(bird_line, aes(male_mass_median, egg_mass_median)) +
         geom_point() +
         scale_x_log10()+
         scale_y_log10() 
...
```{r}
bird_more_5 <- filter (bird_line, n>= 5)
...
```{r}
ggplot(bird_line, aes(male_mass_median, egg_mass_median)) +
         geom_point(position=position_jitter(width = NULL, height = NULL)) +
            geom_point(color= "red", data=bird_more_5) +          
              geom_text_repel(data=bird_more_5, aes(label=type), size=3, color= "red", size= 2, vjust= -.5) +
         scale_x_log10()+
         scale_y_log10() 
```
