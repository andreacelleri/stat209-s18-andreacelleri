...
Answer: There is a positive relationship between the variables. 
...
ggplot(birds, aes(male_mass, egg_mass))+
  ...
  scale_x_log10()+
  scale_y_log10() +
  annotate("text", x= 140, y=6, label="Parrots", size=5, color="red")
...
Answer: Yes, it shows the positive relationship between the bird size and the egg size. 
...
ggplot(birds, aes(male_mass, egg_mass))+
  ...
  scale_y_log10()+
  geom_smooth(method="lm", color="black", se=FALSE, linetype="dashed", size=0.5)+
  annotate("text", x= 218.7, y= 98, label="Tropicbird", size=4)
```
...
polyandry <- filter(birds, mating_system == 1)
polygyny <- filter(birds, mating_system ==4)


ggplot(birds, aes(male_mass, egg_mass))+
  geom_point(alpha=.2)+
  geom_point(data=polygyny, color="red", size=2)+
  scale_x_log10()+
  scale_y_log10()+
  geom_smooth(method="lm", color="black", se=FALSE, linetype="dashed", size=0.5)+
  annotate("text", x= 2450, y= 230, label="Australian Brush-turkey", size=4) +
  labs(x="Male Mass", y= "Egg Mass", title= "Relationship between egg mass, male mass, and polygyny", size=10)+
  annotate("rect", xmin = 3, xmax = 80,
                   ymin = 0, ymax = 6,
                   fill = "red", alpha = 0.1)+
  annotate("text", x = 40, y = .4,
           label = "Polygynous Birds", size=4, color="red")
