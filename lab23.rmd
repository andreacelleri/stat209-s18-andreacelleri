...
arrest_flag <- group_summarize (crimes, arrest_flag)

```

Describe what this means in words.

**Answer**: Out of all the reported crimes 24,331 were arrested with a mean of 0.1724.
...
**Answer**: Out of all the crimes reported, 27% are reported as theft.
...
**Answer**: There are 38,816 reported crimes that were market as theft. 
...
Divide the answer to the previous two question by the sum of the number of
...
**Answer**: For every 36 households there is one crime.
...
**Answer**: It is lower than the mean of the arrest flag of the entire dataset. 
...
**Answer**: This is smaller than the entire dataset. Yes, because area 23 is part of the entire dataset. 
...
**Answer**: It is way larger which does not make sense because there are less households than thefts. 
...
humboldt_park <- filter(ca, area_number =="23")
sum(humboldt_park$num_households)
...
**Answer**: There is a downward trend where the higher the income the lower the theft rate. 
...
ggplot(crimes_ca, aes(median_income, theft_rate))+
  geom_point()+
  geom_smooth()+
  geom_text_repel(data=crimes_ca, aes(label= area_name))
...
**Answer**: The Loop is the central business district and home of the city's commercial core where only 8996 households exist. The low number of households and the commercial aspects of the neighborhood can explain the high levels of theft.  
