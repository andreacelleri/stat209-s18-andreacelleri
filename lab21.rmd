...
**Answer**: Because there is only 20 possible answers so each answer can be considered a category. 

Produce a confidence interval for the mean of your trait.

```{r}

model <- lm_basic(perfectionism ~1, data=pf)
...
country_hu <- filter(pf, country== "HU")

model <- lm_basic(perfectionism ~1, data=country_hu)
reg_table(model, level=0.95)
...
**Answer**: The confidence interval is larger than the original because there are only 55 observations which reduce the certainty of the model. 
...
```{r}
model <- lm_basic(perfectionism ~1 + gender, data= country_hu)
reg_table(model, level=0.95)
```


**Answer**: Males are 0.8764 less perfectionist 
...
age_30_49 <- filter(pf, age >= 30, age <= 49)
...
model<- lm_basic(perfectionism ~1 + fourties, data= pf)
reg_table(model, level=0.95)
...
**Answer**: People in their forties are less perfectionist than those in their thirties. 
...
```{r}
ggplot(pf, aes(fct_lump(country, 5)))+
  geom_bar()+
  coord_flip()
...
**Answer**: Yes. It is interesting because it is positively related, where the higher the baseline, the more sensitive the model. 

Fit a regression model that uses both gender and the baseline score to
predict your trait.

```{r}
model<- lm_basic(perfectionism ~baseline + gender , data= pf)
reg_table(model, level=0.95)
